...
class ThemeState {
  ...
  const ThemeState({this.mode = ThemeMode.dark, this.accentKey = 'teal'});

  AccentPreset get accentPreset => AccentPreset.fromKey(accentKey);

  String get currentModeKey {
    switch (mode) {
  ...
  ThemeState copyWith({ThemeMode? mode, String? accentKey}) {
    return ThemeState(
      mode: mode ?? this.mode,
...
class ThemeCubit extends Cubit<ThemeState> {
  ...
  Future<void> _load() async {
    ...
    final accentKey = prefs.getString('omnivium_accent') ?? 'teal';
    emit(ThemeState(mode: _fromString(savedMode), accentKey: accentKey));
  }
  ...
  Future<void> setAccent(String key) async {
    ...
    emit(state.copyWith(accentKey: key));
  }
