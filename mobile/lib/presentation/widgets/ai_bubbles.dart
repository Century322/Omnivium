          ...
          Flexible(child: Column(
            ...
            children: [
              if (textContent.isNotEmpty)
                Text(textContent, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 15, height: 1.4)),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          if (isRunning)
            SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.accent.withValues(alpha: 0.7)))
          else
            Icon(isSuccess ? LucideIcons.checkCircle2 : LucideIcons.xCircle, size: 14, color: isSuccess ? AppColors.ok(context) : AppColors.dng(context)),
          const SizedBox(width: 8),
          Icon(LucideIcons.wrench, size: 12, color: AppColors.sec(context).withValues(alpha: 0.7)),
          const SizedBox(width: 4),
          Text(log.skillName, style: TextStyle(color: AppColors.textHint(context), fontSize: 12, fontWeight: FontWeight.w500)),
          if (!isRunning) ...[
            const SizedBox(width: 8),
            Text('${duration}ms', style: TextStyle(color: AppColors.iconGray(context), fontSize: 11)),
          ],
          if (isRunning) ...[
            const SizedBox(width: 8),
            Text(localeProvider.t('execution_running'), style: TextStyle(color: AppColors.accent, fontSize: 11)),
          ],
        ],
      ),
    );
...
class AiActionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      _actionIcon(context, LucideIcons.rotateCcw, localeProvider.t('regenerate'), onTap: onRegenerate),
      const SizedBox(width: 20),
      _actionIcon(context, LucideIcons.clipboardCopy, localeProvider.t('copy'), onTap: onCopy),
      const SizedBox(width: 20),
      _actionIcon(context, LucideIcons.headphones, localeProvider.t('listen'), onTap: onSpeak),
      const SizedBox(width: 20),
      _actionIcon(context, LucideIcons.share, localeProvider.t('share'), onTap: onShare),
      const Spacer(),
      _actionIcon(context, LucideIcons.moreVertical, localeProvider.t('more'), onTap: onMore),
    ]);
  }
  ...
  static Widget _actionIcon(BuildContext context, IconData icon, String label, {required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 4),
        Text(label, style: TextStyle(color: AppColors.sec(context), fontSize: 11)),
      ]),
    );
  }
}
