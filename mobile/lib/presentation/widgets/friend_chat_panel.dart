...
class _FriendChatPanelState extends State<FriendChatPanel> with TickerProviderStateMixin {
  ...
  void _onMatrixChanged() {
    ...
    if (room != null) {
      final typingUsers = room.typingUsers.where((u) => u.id != matrix.userId).toList();
    ...
    if (relevantEvents.isNotEmpty) {
      for (final event in relevantEvents) {
        if (event.type == EventTypes.Message && event.body.isNotEmpty) {
          ...
          final url = event.content['url'] as String? ?? (event.content['file'] is Map ? (event.content['file'] as Map)['url'] as String? : null);
          final audioDuration = event.content['info'] is Map ? (event.content['info'] as Map)['duration'] as int? : null;
          setState(() {
            _friendMessages.add(FriendMessageData(isMe: isMe, content: event.body, msgType: msgType, url: url, audioDuration: audioDuration));
          });
        }
      }
      ...
      setState(() {
        ...
        for (final event in timeline.events) {
          if (event.type == EventTypes.Message && event.body.isNotEmpty) {
            ...
            final url = event.content['url'] as String? ?? (event.content['file'] is Map ? (event.content['file'] as Map)['url'] as String? : null);
            final audioDuration = event.content['info'] is Map ? (event.content['info'] as Map)['duration'] as int? : null;
            _friendMessages.add(FriendMessageData(isMe: isMe, content: event.body, eventId: event.eventId, msgType: msgType, url: url, audioDuration: audioDuration));
          }
        }
      });
    } catch (e, stackTrace) {
  ...
  void _sendTypingNotice() {
    ...
    final room = widget.provider.matrix.client?.getRoomById(widget.chatTargetId);
    if (room == null) return;
  ...
  void _sendFileMessage(String prefix, String fileName, String? filePath) {
    if (!mounted) return;
    setState(() {
      _friendMessages.add(FriendMessageData(isMe: true, content: '$prefix $fileName'));
    });
    final matrix = widget.provider.matrix;
    if (matrix.isLoggedIn && widget.chatTargetId.isNotEmpty && filePath != null) {
      if (prefix.contains(localeProvider.t('photo_msg')) || prefix.contains(localeProvider.t('camera_msg'))) {
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              title: Text(t('copy'), style: TextStyle(color: AppColors.textPrimary(context))),
              onTap: () {
            ...
            if (msg.isMe) ListTile(
              leading: Icon(LucideIcons.rotateCcw, color: AppColors.warn(context)),
              title: Text(t('recall'), style: TextStyle(color: AppColors.warn(context))),
              onTap: () {
                Navigator.pop(context);
  ...
  void _recallMessage(int index) {
    ...
    setState(() {
      _friendMessages[index] = FriendMessageData(isMe: msg.isMe, content: t('message_recalled'), eventId: msg.eventId);
    });
  }
  ...
  void _showFriendChatMenu(BuildContext ctx) {
    showMenu(
      ...
      items: [
        ...
        _menuItem(LucideIcons.trash2, localeProvider.t('clear_chat'), isDanger: true),
      ],
    ).then((value) {
  ...
  void _toggleMute() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(isMuted ? localeProvider.t('muted_off') : localeProvider.t('muted_on')), duration: const Duration(seconds: 2)),
    );
  }
  ...
  void _showFriendProfile() {
    ...
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => FriendProfileView(provider: widget.provider, roomId: widget.chatTargetId),
    ));
  }

  void _showEncryptionInfo() {
    final matrix = widget.provider.matrix;
    final room = widget.chatTargetId.isNotEmpty ? matrix.client?.getRoomById(widget.chatTargetId) : null;
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          Icon(isEncrypted ? LucideIcons.shieldCheck : LucideIcons.shieldAlert,
              color: isEncrypted ? AppColors.accent : AppColors.warn(context), size: 20),
          const SizedBox(width: 8),
          Text(isEncrypted ? localeProvider.t('e2e_encrypted_short') : localeProvider.t('not_encrypted_short'),
        ...
        content: Column(
          ...
          children: [
            if (isEncrypted) ...[
              ...
              Text(t('encrypt_verify'), style: TextStyle(color: AppColors.textPrimary(context), fontSize: 15, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              FutureBuilder<String?>(
                ...
                builder: (context, snap) {
                  ...
                  if (emojis == null || emojis.isEmpty) {
                    return Text(t('verify_unavailable'), style: TextStyle(color: AppColors.textHint(context), fontSize: 13));
                  }
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(emojis, style: TextStyle(fontSize: 28, letterSpacing: 4)),
                      const SizedBox(height: 6),
                      Text(t('verify_instruction'),
        ...
        actions: [
          if (isEncrypted)
            TextButton(
              ...
              child: Text(t('verify_device'), style: TextStyle(color: AppColors.accent)),
            ),
          TextButton(
  ...
  Future<CachedPresence?> _getPresence() async {
    ...
    try {
      return await client.fetchCurrentPresence(otherUser.id, fetchOnlyFromCached: true);
    } catch (e, stackTrace) {
      AppLogger.instance.warning('Operation failed', error: e, stackTrace: stackTrace);
      return null;
    }
  ...
  void _shareFriendConversation() {
    ...
    final text = _friendMessages.map((m) => m.isMe ? '${localeProvider.t('me')}：${m.content}' : '${widget.chatTargetName}：${m.content}').join('\n\n');
  ...
  void _clearFriendChat() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: AppColors.sf(context),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text(t('clear_chat'), style: TextStyle(color: AppColors.textPrimary(context))),
        content: Text(t('clear_chat_confirm'), style: TextStyle(color: AppColors.textSecondary(context))),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            ...
            if (query.isNotEmpty) {
              for (var i = 0; i < _friendMessages.length; i++) {
                if (_friendMessages[i].content.toLowerCase().contains(query)) {
                  results.add({'index': i, 'content': _friendMessages[i].content, 'isMe': _friendMessages[i].isMe});
                }
              }
            }
            return Padding(
                  ...
                  children: [
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                prefixIcon: Icon(LucideIcons.search, color: AppColors.textHint(context), size: 18),
                                filled: true,
                    ...
                    if (query.isNotEmpty && results.isEmpty)
                      Padding(
                        ...
                        child: Text(t('no_match_msg'), style: TextStyle(color: AppColors.textDisabled(context), fontSize: 14)),
                      ),
                      ...
                      ConstrainedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            final highlightStart = content.toLowerCase().indexOf(query);
                            return ListTile(
                              dense: true,
                              title: _buildHighlightedText(content, highlightStart, query.length),
                              subtitle: Text(
                                r['isMe'] as bool ? localeProvider.t('me') : widget.chatTargetName,
  ...
  Widget _buildHighlightedText(String text, int start, int length) {
    if (start < 0) return Text(text, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 14));
    ...
    return RichText(
      ...
      text: TextSpan(
        ...
        children: [
          ...
          TextSpan(text: match, style: TextStyle(color: AppColors.accent, fontWeight: FontWeight.w600)),
          TextSpan(text: after),
        ],
      ),
    );
  ...
  PopupMenuItem<String> _menuItem(IconData icon, String text, {bool isDanger = false}) {
    return PopupMenuItem<String>(
      ...
      child: Row(children: [
        ...
        Text(text, style: TextStyle(
          color: isDanger ? AppColors.dng(context) : AppColors.textPrimary(context),
          fontSize: 14,
      ...
      behavior: HitTestBehavior.opaque,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.chatTargetName, style: TextStyle(color: AppColors.textPrimary(context), fontSize: 16, fontWeight: FontWeight.w600)),
                    if (_isOtherTyping)
                      Text(t('typing'), style: TextStyle(color: AppColors.accent, fontSize: 11))
                    else
                      FutureBuilder<CachedPresence?>(
                        future: _presenceFuture,
                        builder: (context, snap) {
                          final presence = snap.data;
                          if (presence == null) return Text('@${widget.chatTargetId}', style: TextStyle(color: AppColors.textTertiary(context), fontSize: 11));
                          final isOnline = presence.currentlyActive == true;
                          final status = isOnline ? localeProvider.t('online') : localeProvider.t('offline');
                          return Row(children: [
                            Container(width: 6, height: 6, decoration: BoxDecoration(color: isOnline ? AppColors.accent : AppColors.iconGray(context), borderRadius: BorderRadius.circular(3))),
                            const SizedBox(width: 4),
                            Text(status, style: TextStyle(color: isOnline ? AppColors.accent : AppColors.iconGray(context), fontSize: 11)),
                          ]);
                        },
                      ),
                  ],
    ...
    return ListView.builder(
      ...
      behavior: HitTestBehavior.opaque,
              ...
              child: Container(
                ...
                padding: msg.isImage || msg.isFile ? EdgeInsets.zero : const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                decoration: BoxDecoration(
                  color: msg.isMe ? AppColors.sfHover(context) : AppColors.sf(context),
                  ...
                  children: [
                    if (msg.isVoice && msg.url != null)
                      ...
                      Text(msg.content, style: TextStyle(
                        color: msg.isMe ? AppColors.textPrimary(context) : AppColors.textSecondary(context),
                        fontSize: 15,
  ...
  Widget _buildImageBubble(FriendMessageData msg) {
    ...
    if (mxcUrl == null) return Text(msg.content, style: TextStyle(color: AppColors.textPrimary(context), fontSize: 14));
    ...
    return ClipRRect(
      ...
      behavior: HitTestBehavior.opaque,
        ...
        child: httpUrl != null
            ? CachedNetworkImage(imageUrl: httpUrl, width: 200, fit: BoxFit.cover, placeholder: (_, _) => Container(width: 200, height: 150, color: AppColors.sfAlt(context), child: Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.accent)))), errorWidget: (_, _, _) => Container(width: 200, height: 80, color: AppColors.sfAlt(context), child: Icon(LucideIcons.imageOff, color: AppColors.iconGray(context))))
            : Container(width: 200, height: 80, color: AppColors.sfAlt(context), child: Center(child: Icon(LucideIcons.image, color: AppColors.iconGray(context)))),
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.sfAlt(context), borderRadius: BorderRadius.circular(10)), child: Icon(LucideIcons.download, color: AppColors.accent, size: 20)),
          const SizedBox(width: 10),
          Flexible(child: Text(msg.content, style: TextStyle(color: AppColors.textPrimary(context), fontSize: 14), maxLines: 2, overflow: TextOverflow.ellipsis)),
        ]),
      ),
    );
  }
    ...
    return Padding(
                    ...
                    child: Semantics(
                      ...
                      child: Padding(
                        ...
                        child: Container(
                          ...
                          height: 12,
                          decoration: BoxDecoration(
                              color: AppColors.textPrimary(context), borderRadius: BorderRadius.circular(2)),
                        ),
                      ),
                    ),
                  )
                : Row(key: const ValueKey('normal-left'), children: [
                    _circleBtn(LucideIcons.plus, onTap: () => _showPlusMenu()),
                  ]),
          ),
          AnimatedSwitcher(
            duration: const Duration(milliseconds: 300),
      ...
      behavior: HitTestBehavior.opaque,
                ...
                : Row(key: const ValueKey('normal-right'), children: [
                    VoiceRecorderButton(
                      onSend: (path, duration) => _sendVoiceMessage(path, duration),
                    ),
