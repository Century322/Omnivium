...
class _SearchViewState extends State<SearchView> {
  ...
  Future<void> _saveHistory(String query) async {
    final prefs = await SharedPreferences.getInstance();
    _searchHistory = [query, ..._searchHistory.where((h) => h != query)].take(10).toList();
      ...
      try {
        ...
        for (final user in users.take(5)) {
          results.add(_SearchResult(
            ...
            title: user.displayName ?? user.userId.split(':').first.replaceFirst('@', ''),
    ...
    for (final session in widget.provider.session.sessions.take(20)) {
      if (session.title.toLowerCase().contains(query.toLowerCase())) {
        results.add(_SearchResult(
          ...
          title: session.title,
          subtitle: '${session.messages.length} ${localeProvider.t('messages')}',
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: _isSearching
                  ? SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: List.generate(4, (_) => const CardSkeleton())))
                  : _query.isEmpty
                ...
                children: [
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      autofocus: true,
                      style: TextStyle(color: AppColors.textPrimary(context), fontSize: 15, fontWeight: FontWeight.w500),
  ...
  Widget _buildHistorySection(BuildContext context) {
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(LucideIcons.search, color: AppColors.textDisabled(context), size: 28),
            ),
            const SizedBox(height: 16),
            Text(localeProvider.t('no_search_history'), style: TextStyle(fontWeight: FontWeight.w500, color: AppColors.textHint(context))),
            const SizedBox(height: 4),
            Text(localeProvider.t('search_history_desc'), style: TextStyle(fontSize: 12, color: AppColors.textDisabled(context))),
          ],
        ),
      );
    }
    ...
    return ListView(
      ...
      behavior: HitTestBehavior.opaque,
        ...
        ..._searchHistory.map((h) => ListTile(
              leading: Icon(LucideIcons.clock, size: 18, color: AppColors.textDisabled(context)),
              title: Text(h, style: TextStyle(color: AppColors.textPrimary(context), fontSize: 14)),
              trailing: Icon(LucideIcons.arrowUpRight, size: 16, color: AppColors.textDisabled(context)),
              onTap: () {
    ...
    if (_results.isEmpty) {
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(LucideIcons.searchX, size: 48, color: AppColors.textDisabled(context)),
            const SizedBox(height: 12),
            Text(localeProvider.t('no_search_results'), style: TextStyle(color: AppColors.textHint(context), fontWeight: FontWeight.w500)),
          ],
        ),
      );
    }

    final messages = _results.where((r) => r.type == _SearchResultType.message).toList();
  ...
  Widget _buildSectionHeader(BuildContext context, String title, int count) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 4),
      child: Text('$title ($count)', style: TextStyle(color: AppColors.mut(context), fontSize: 12, fontWeight: FontWeight.w600)),
    );
  }
  ...
  Widget _buildResultTile(BuildContext context, _SearchResult result) {
    ...
    return ListTile(
      ...
      title: Text(result.title, style: TextStyle(color: AppColors.textPrimary(context), fontWeight: FontWeight.w500, fontSize: 14)),
      subtitle: Text(result.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: AppColors.mut(context), fontSize: 12)),
    ...
    switch (result.type) {
      ...
      case _SearchResultType.user:
        if (result.userId != null) {
          widget.provider.matrix.createDirectChat(result.userId!).then((roomId) {
