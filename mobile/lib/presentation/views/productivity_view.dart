...
class _ProductivityViewState extends State<ProductivityView> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(t('productivity'), style: TextStyle(color: AppColors.textPrimary(context), fontSize: 17, fontWeight: FontWeight.w600)),
        bottom: TabBar(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(text, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 15)),
        ],
      ),
    );
  }
  ...
  Widget _buildNoteCard(NoteItem note) {
    return Container(
      ...
      child: ListTile(
        title: Text(note.title, style: TextStyle(color: AppColors.textPrimary(context), fontSize: 15, fontWeight: FontWeight.w500)),
        subtitle: note.content.isNotEmpty ? Text(note.content, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 13), maxLines: 2, overflow: TextOverflow.ellipsis) : null,
        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
          IconButton(tooltip: localeProvider.t('edit'), icon: Icon(LucideIcons.pencil, size: 16, color: AppColors.textSecondary(context)), onPressed: () => _showEditDialog(note)),
          IconButton(tooltip: localeProvider.t('delete'), icon: Icon(LucideIcons.trash2, size: 16, color: AppColors.dng(context).withValues(alpha: 0.7)), onPressed: () => widget.provider.deleteItem(note.id)),
        ]),
      ),
    );
  }
      ...
      behavior: HitTestBehavior.opaque,
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: todo.isDone ? AppColors.accent : AppColors.textSecondary(context), width: 2),
              borderRadius: BorderRadius.circular(6),
            ),
            child: todo.isDone ? Icon(LucideIcons.check, size: 14, color: AppColors.textPrimary(context)) : null,
          ),
        )),
        title: Text(todo.title, style: TextStyle(
          color: todo.isDone ? AppColors.textDisabled(context) : AppColors.textPrimary(context),
          fontSize: 15,
  ...
  Widget _buildScheduleCard(NoteItem schedule) {
    final isOverdue = schedule.dueDate != null && schedule.dueDate!.isBefore(DateTime.now());
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 3),
      decoration: BoxDecoration(
        color: AppColors.sf(context),
        borderRadius: BorderRadius.circular(12),
        border: isOverdue ? Border.all(color: AppColors.dng(context).withValues(alpha: 0.3), width: 1) : null,
      ),
      child: ListTile(
        leading: Container(
          width: 44, height: 44,
          decoration: BoxDecoration(color: isOverdue ? AppColors.dng(context).withValues(alpha: 0.1) : AppColors.accent.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(schedule.dueDate != null ? '${schedule.dueDate!.day}' : '-', style: TextStyle(color: isOverdue ? AppColors.dng(context) : AppColors.accent, fontSize: 16, fontWeight: FontWeight.w700)),
            Text(schedule.dueDate != null ? _monthShort(schedule.dueDate!.month) : '', style: TextStyle(color: isOverdue ? AppColors.dng(context).withValues(alpha: 0.7) : AppColors.accent.withValues(alpha: 0.7), fontSize: 9, fontWeight: FontWeight.w600)),
          ]),
        ),
  ...
  void _showAddDialog(int tabIndex) {
    final type = tabIndex == 0 ? NoteType.text : tabIndex == 1 ? NoteType.todo : NoteType.schedule;
    ...
    showDialog(
        ...
        return StatefulBuilder(
          builder: (ctx, setDialogState) {
            return AlertDialog(
              ...
              title: Text(
                tabIndex == 0 ? t('add_note') : tabIndex == 1 ? t('add_todo') : t('add_schedule'),
                    ...
                    children: [
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          filled: true, fillColor: AppColors.sfAlt(context),
                          border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                          contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                        ),
                      ),
                      const SizedBox(height: 12),
                      if (type == NoteType.text) TextField(
      ...
      behavior: HitTestBehavior.opaque,
                          ...
                          child: Container(
                            ...
                            child: Row(children: [
                              Icon(LucideIcons.calendar, size: 16, color: AppColors.textSecondary(context)),
                              const SizedBox(width: 8),
                              Text(dueDate != null ? _formatDateTime(dueDate!) : t('select_date'), style: TextStyle(color: dueDate != null ? AppColors.textPrimary(context) : AppColors.textDisabled(context), fontSize: 14)),
                            ]),
                          ),
                        )),
                        const SizedBox(height: 8),
      ...
      behavior: HitTestBehavior.opaque,
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: AppColors.sfAlt(context), borderRadius: BorderRadius.circular(10)),
                            child: Row(children: [
                              Icon(LucideIcons.clock, size: 16, color: AppColors.textSecondary(context)),
                              const SizedBox(width: 8),
                              Text(dueTime != null ? '${dueTime!.hour.toString().padLeft(2, '0')}:${dueTime!.minute.toString().padLeft(2, '0')}' : t('select_time'), style: TextStyle(color: dueTime != null ? AppColors.textPrimary(context) : AppColors.textDisabled(context), fontSize: 14)),
                            ]),
                          ),
                        )),
                      ],
                  ...
                  onPressed: () async {
                    ...
                    if (type == NoteType.schedule && dueDate != null) {
                      finalDueDate = DateTime(
                        ...
                        dueTime?.hour ?? 0, dueTime?.minute ?? 0,
                      );
                    }
                    final item = NoteItem(
    ...
    showDialog(
      ...
      builder: (ctx) {
        return AlertDialog(
          backgroundColor: AppColors.sf(context),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: Text(t('edit_note'), style: TextStyle(color: AppColors.textPrimary(context), fontSize: 17, fontWeight: FontWeight.w600)),
          content: SizedBox(
            width: MediaQuery.of(context).size.width * 0.85,
            child: Column(
              mainAxisSize: MainAxisSize.min,
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                final title = titleCtrl.text.trim();
                if (title.isEmpty) return;
                final updated = note.copyWith(title: title, content: contentCtrl.text.trim());
