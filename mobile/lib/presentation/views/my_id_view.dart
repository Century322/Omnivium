...
class MyIdView extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          Text(
            userId != notLoggedIn ? userId.split(':').first.replaceAll('@', '') : notLoggedIn,
  ...
  Widget _buildAvatar(BuildContext context, String userId, String notLoggedIn, dynamic identity) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: identity != null
            ? [BoxShadow(color: borderColor.withValues(alpha: 0.15), blurRadius: 12, spreadRadius: 2)]
            : null,
      ),
      child: Center(
        child: Text(
          userId != notLoggedIn && userId.isNotEmpty ? userId[1].toUpperCase() : '?',
          style: TextStyle(color: borderColor, fontSize: 42, fontWeight: FontWeight.w700),
        ),
      ),
    );
  }

  Widget _buildMatrixIdCard(BuildContext context, String userId, String notLoggedIn) {
    final t = localeProvider.t;
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(LucideIcons.messageCircle, size: 14, color: AppColors.sec(context)),
          const SizedBox(width: 6),
          Text(t('matrix_id'), style: TextStyle(color: AppColors.textHint(context), fontSize: 12, fontWeight: FontWeight.w600)),
        ]),
          ...
          child: OutlinedButton.icon(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(t('id_copied')), backgroundColor: AppColors.accent, duration: const Duration(milliseconds: 1500)),
              );
            },
          ),
        ),
  ...
  Widget _buildSovereignIdCard(BuildContext context, dynamic identity) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(LucideIcons.shieldCheck, size: 14, color: AppColors.acc(context)),
          const SizedBox(width: 6),
          Text(t('sovereign_identity'), style: TextStyle(color: AppColors.acc(context), fontSize: 12, fontWeight: FontWeight.w600)),
        ]),
        const SizedBox(height: 16),
        _buildInfoRow(context, t('did'), did, LucideIcons.fingerprint, AppColors.acc(context)),
        const SizedBox(height: 10),
        _buildInfoRow(context, t('node_id'), nodeId, LucideIcons.server, AppColors.sec(context)),
        const SizedBox(height: 10),
        _buildInfoRow(context, t('public_key'), _truncateKey(publicKey), LucideIcons.key, AppColors.warn(context)),
        const SizedBox(height: 10),
        _buildInfoRow(context, t('civilization_epoch'), '$epoch', LucideIcons.layers, AppColors.ok(context)),
        if (federationId != null) ...[
          const SizedBox(height: 10),
          _buildInfoRow(context, t('federation_id'), federationId, LucideIcons.globe, AppColors.sec(context)),
        ],
        const SizedBox(height: 10),
        _buildInfoRow(context, t('created_at'), _formatDate(createdAt), LucideIcons.calendar, AppColors.textTertiary(context)),
        const SizedBox(height: 12),
        SizedBox(
          width: double.infinity,
          child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(
              ...
              side: BorderSide(color: AppColors.acc(context).withValues(alpha: 0.5)),
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(t('did_copied')), backgroundColor: AppColors.acc(context), duration: const Duration(milliseconds: 1500)),
              );
            },
          ),
        ),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 6),
          Text(t('trust_level'), style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.w600)),
          const Spacer(),
          Container(
        ...
        if (ancestry.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text(t('ancestry'), style: TextStyle(color: AppColors.textHint(context), fontSize: 11, fontWeight: FontWeight.w600)),
          const SizedBox(height: 4),
          Wrap(
            spacing: 4,
            runSpacing: 4,
            children: ancestry.map((a) => Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(
                color: AppColors.sfAlt(context),
                borderRadius: BorderRadius.circular(4),
              ),
              child: Text('$a', style: TextStyle(color: AppColors.textTertiary(context), fontSize: 10)),
            )).toList(),
          ),
        ],
      ]),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 6),
          Text(t('credentials'), style: TextStyle(color: AppColors.acc(context), fontSize: 12, fontWeight: FontWeight.w600)),
          const Spacer(),
          Text('${credentials.length}', style: TextStyle(color: AppColors.textTertiary(context), fontSize: 13, fontWeight: FontWeight.w600)),
        ]),
        if (credentials.isEmpty) ...[
          const SizedBox(height: 8),
          Text(t('no_credentials'), style: TextStyle(color: AppColors.textTertiary(context), fontSize: 12)),
        ],
        ...credentials.take(3).map((c) => Padding(
          padding: const EdgeInsets.only(top: 8),
          child: Row(children: [
            Icon(LucideIcons.badgeCheck, size: 14, color: AppColors.ok(context)),
            const SizedBox(width: 6),
            Expanded(child: Text(c.credentialType, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 12), overflow: TextOverflow.ellipsis)),
          ]),
        )),
      ]),
    );
  ...
  Widget _buildNoIdentityCard(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.sf(context),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: AppColors.warn(context).withValues(alpha: 0.2)),
      ),
      child: Column(children: [
        Icon(LucideIcons.shieldOff, size: 32, color: AppColors.warn(context)),
        const SizedBox(height: 8),
        Text(t('no_sovereign_identity'), style: TextStyle(color: AppColors.textSecondary(context), fontSize: 14)),
        const SizedBox(height: 4),
        Text(t('no_sovereign_identity_desc'), style: TextStyle(color: AppColors.textTertiary(context), fontSize: 12), textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  Widget _buildShareCard(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        _shareOption(context, LucideIcons.messageCircle, t('add_via_omnivium'), t('add_via_omnivium_desc'), onTap: () {
          Clipboard.setData(ClipboardData(text: 'omnivium://add?id=$shareId'));
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(t('copied'))));
        }),
        const SizedBox(height: 8),
        _shareOption(context, LucideIcons.link, t('share_via_link'), t('share_via_link_desc'), onTap: () {
          Share.share('https://omnivium.app/i/${Uri.encodeComponent(shareId)}', subject: t('add_me_omnivium'));
        }),
        const SizedBox(height: 8),
        _shareOption(context, LucideIcons.qrCode, t('qr_code'), t('qr_code_desc'), onTap: () {
  ...
  Widget _buildInfoRow(BuildContext context, String label, String value, IconData icon, Color color) {
    return Row(children: [
      ...
      const SizedBox(width: 6),
      Text(label, style: TextStyle(color: AppColors.textTertiary(context), fontSize: 11, fontWeight: FontWeight.w600)),
      const Spacer(),
      Flexible(
        child: Text(value, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 12, fontFamily: 'monospace'),
          overflow: TextOverflow.ellipsis, textAlign: TextAlign.end),
      ),
    ]);
  }

  Widget _shareOption(BuildContext context, IconData icon, String title, String subtitle, {VoidCallback? onTap}) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: AppColors.sfAlt(context), borderRadius: BorderRadius.circular(10)),
      child: Row(children: [
        Container(
          width: 36, height: 36,
  ...
  void _showQrDialog(BuildContext context, String shareId) {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: Text(t('qr_code'), style: TextStyle(color: AppColors.textPrimary(context))),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 12),
        Text(shareId, style: TextStyle(color: AppColors.textSecondary(context), fontSize: 11, fontFamily: 'monospace'),
