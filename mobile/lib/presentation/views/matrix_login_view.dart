...
class MatrixLoginView extends StatefulWidget {
  ...
  const MatrixLoginView({super.key, required this.provider, this.onLoginSuccess});
...
class _MatrixLoginViewState extends State<MatrixLoginView> {
  ...
  Future<String> _getHomeserver() async {
    if (_showAdvanced && _homeserverCtrl.text.trim().isNotEmpty) return _homeserverCtrl.text.trim();
  ...
  Future<void> _submit() async {
    ...
    if (password.length < 8) { setState(() => _error = localeProvider.t('password_min_length')); return; }
    if (username.length > 255 || password.length > 1024) { setState(() => _error = localeProvider.t('input_too_long')); return; }
  ...
  Widget build(BuildContext context) {
    final t = localeProvider.t;
    return Scaffold(
      body: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.symmetric(horizontal: 32),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const SizedBox(height: 40),
          Center(child: Container(width: 80, height: 80,
            decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.accent, AppColors.accentPurple], begin: Alignment.topLeft, end: Alignment.bottomRight),
              borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: AppColors.accent.withValues(alpha: 0.3), blurRadius: 24, offset: const Offset(0, 8))]),
            child: Icon(LucideIcons.messageCircle, size: 40, color: AppColors.textPrimary(context)))),
          const SizedBox(height: 24),
          Center(child: Text('Omnivium', style: TextStyle(color: AppColors.textPrimary(context), fontSize: 28, fontWeight: FontWeight.w800, letterSpacing: -0.5))),
          const SizedBox(height: 4),
          Center(child: Text(t('e2e_encrypted'), style: TextStyle(color: AppColors.textHint(context), fontSize: 14))),
          ...
          if (_error != null) ...[const SizedBox(height: 12),
            Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.dng(context).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
              child: Text(_error!, style: TextStyle(color: AppColors.dng(context), fontSize: 13)))],
          const SizedBox(height: 24),
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: AppColors.accent, foregroundColor: AppColors.textPrimary(context),
            ...
            child: _isLoading ? SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.textPrimary(context)))
                : Text(_isRegister ? t('create_account') : t('login'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700))),
          const SizedBox(height: 20),
          Center(child: Semantics(label: localeProvider.t('switch_to_register'), child: GestureDetector(onTap: () => setState(() { _isRegister = !_isRegister; _error = null; }),
            child: Text(_isRegister ? t('have_account_login') : t('no_account_create'), style: TextStyle(color: AppColors.accent, fontSize: 14, fontWeight: FontWeight.w500))))),
          const SizedBox(height: 40),
        ])))));
  }
}
