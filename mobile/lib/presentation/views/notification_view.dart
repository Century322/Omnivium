  ...
  Widget _buildBody(BuildContext context, String Function(String) t) {
    ...
    if (notifications.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(LucideIcons.bell, size: 48, color: AppColors.textDisabled(context)),
        const SizedBox(height: 16),
        Text(t('no_notifications'), style: TextStyle(color: AppColors.textTertiary(context), fontSize: 15)),
      ]));
    }
      ...
      child: GestureDetector(
        ...
        child: Container(padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: notif.read ? AppColors.sf(context) : AppColors.sfAlt(context), borderRadius: BorderRadius.circular(12),
            border: notif.read ? null : Border.all(color: AppColors.accent.withValues(alpha: 0.2))),
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 36, height: 36, decoration: BoxDecoration(color: iconColor.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(18)),
              child: Icon(icon, size: 18, color: iconColor)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(notif.title, style: TextStyle(color: notif.read ? AppColors.textHint(context) : AppColors.textPrimary(context), fontSize: 14, fontWeight: notif.read ? FontWeight.w400 : FontWeight.w600), overflow: TextOverflow.ellipsis)),
                Text(formatRelativeTime(notif.timestamp), style: TextStyle(color: AppColors.iconGray(context), fontSize: 11)),
              ]),
              const SizedBox(height: 4),
              Text(notif.body, style: TextStyle(color: AppColors.textHint(context), fontSize: 13), maxLines: 2, overflow: TextOverflow.ellipsis),
            ])),
