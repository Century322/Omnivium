...
class RuntimeSpecRegistry {
  ...
  List<RuntimeRfc> get activeRfcs =>
      _rfcs.values.where((r) => r.status == RfcStatus.accepted || r.status == RfcStatus.frozen).toList();
  ...
  void _registerFrozenRfcs() {
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'Defines the lifecycle of Runtime: boot → running → shutdown. '
          'All components must respect this lifecycle. No component may operate outside it.',
      motivation: 'Without a defined lifecycle, components can initialize in arbitrary order, '
          'leading to dependency violations and race conditions.',
      specification: '1. RuntimeContainer.boot() transitions state: uninitialized → booting → running.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'Capabilities are discovered through CapabilityRouter. '
          'Invocation goes through: discover → policy check → resource check → invoke.',
      motivation: 'Direct plugin-to-plugin invocation creates hidden dependencies. '
          'Capability-based routing enables governance, policy enforcement, and remote invocation.',
      specification: '1. All capabilities must be declared in PluginDescriptor.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'Defines the frame format, handshake, ack, heartbeat, and chunking protocol '
          'for inter-node communication.',
      motivation: 'Without a defined wire protocol, each transport implements its own framing, '
          'leading to incompatible message formats and unreliable delivery.',
      specification: '1. All communication uses WireFrame with: frameId, type, source, target, hlcTime.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'Unifies Session Lease, Resource Ownership, and Capability Lock '
          'into a single Lease Semantics with acquire/renew/release/revoke lifecycle.',
      motivation: 'Three independent lease-like mechanisms (session lease, resource budget, '
      ...
      specification: '1. UnifiedLease has type: session | resource | capability.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: '7 frozen distributed invariants: Per-Session Ordering, Differentiated Delivery, '
          'Single Writer Session, Eventually Consistent Capabilities, HLC Time Authority, '
          'Node Isolation, Network Is Hostile.',
      motivation: 'Distributed systems without explicit invariants inevitably develop inconsistencies. '
          'Defining invariants before implementation prevents semantic collapse.',
      specification: '1. Message Ordering: Per-Session Ordering. Cross-session not guaranteed.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'All state mutations are recorded in a WAL before execution. '
          'Supports transactions, checkpoint, compaction, and replay.',
      motivation: 'Without a WAL, crash recovery is impossible. Event sourcing requires '
          'a durable, ordered log of all state changes.',
      specification: '1. Every state mutation appends a WalEntry with LSN, type, HLC time, data.\n'
    ...
    register(const RuntimeRfc(
      ...
      createdAt: 0,
      frozenAt: 0,
      summary: 'Defines trust levels, plugin signing, capability authorization, '
          'secret management, and audit logging.',
      motivation: 'As Runtime opens to external plugins and remote nodes, '
          'a security model is essential to prevent unauthorized access and resource abuse.',
      specification: '1. Trust levels: system > signed > verified > untrusted > blocked.\n'
    ...
    register(const RuntimeRfc(
      ...
      author: 'runtime-team',
      createdAt: 0,
      summary: 'Defines Sandbox as a governed resource universe. '
      ...
      motivation: 'AI Runtime will execute untrusted code: third-party plugins, '
      ...
      specification: '1. SandboxIsolate: every execution unit runs inside an isolate with its own resource universe.\n'
      ...
      breakingChanges: 'None. Sandbox is additive. Existing plugins continue to work in the default trust boundary.',
      migrationGuide: 'Existing plugins automatically run in the "default" trust boundary with verified trust level. '
