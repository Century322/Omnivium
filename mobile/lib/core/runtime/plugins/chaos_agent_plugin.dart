...
class ChaosAgentPlugin implements PluginHandler {
    ...
    switch (capabilityId) {
      ...
      default:
        return CapabilityResult.fail(
          RuntimeError(code: 'UNKNOWN_CAPABILITY', message: 'Unknown: $capabilityId'),
        );
    }
  }
  ...
  CapabilityResult _malformedStream(CapabilityContext context) {
    final (stream, controller) = RuntimeStream.create(
      ...
      source: RuntimeRoute(capability: 'chaos.malformed_stream', pluginId: 'chaos-agent'),
    );
  ...
  CapabilityResult _partialFailure(dynamic params) {
    final failRate = params is Map ? (params['failRate'] as num?)?.toDouble() ?? 0.5 : 0.5;
    final shouldFail = _random.nextDouble() < failRate;
    if (shouldFail) {
      return CapabilityResult.fail(
        const RuntimeError(code: 'CHAOS_PARTIAL', message: 'Random partial failure', recoverable: true),
      );
    }
  ...
  CapabilityResult _infiniteStream(CapabilityContext context) {
    final (stream, controller) = RuntimeStream.create(
      ...
      source: RuntimeRoute(capability: 'chaos.infinite_stream', pluginId: 'chaos-agent'),    );
  ...
  Future<CapabilityResult> _retryStorm(dynamic params) async {
    final maxRetries = params is Map ? (params['maxRetries'] as int?) ?? 100 : 100;
    return CapabilityResult.fail(
      RuntimeError(
  ...
  CapabilityResult _eventFlood(dynamic params, CapabilityContext context) {
    ...
    return CapabilityResult.ok({'eventsToEmit': count, 'instruction': 'Use EventBus.publish in loop from test harness'});
  }

  Future<CapabilityResult> _recursiveTool(dynamic params, CapabilityContext context) async {
    ...
    return CapabilityResult.ok({
      'depth': depth,
      'instruction': 'Test harness should call chaos.recursive_tool with depth=${depth - 1}',
    });
  }
  ...
  CapabilityResult _memoryPressure(dynamic params) {
    ...
    return CapabilityResult.ok({'allocated': '${sizeKb}KB', 'length': data.length});
  }
  ...
  Future<CapabilityResult> _cancelTest(CapabilityContext context) async {
    ...
    if (context.shouldAbort) {
      ...
      return CapabilityResult.fail(
        const RuntimeError(code: 'CANCELLED', message: 'Operation cancelled', recoverable: false),
      );
    }
    steps.add('completed');
  ...
  static PluginDescriptor descriptor() => PluginDescriptor(
        ...
        version: '1.0.0',
        description: 'Runtime torture tool - generates timeout, malformed stream, partial failure, infinite stream, retry storm, event flood, crash',
        capabilities: const [
