...
import 'plugin/plugin_handler.dart' show PluginHandler, CapabilityResult, RuntimeError;
...
class CapabilityRouter {
  ...
  List<CapabilityBinding> get allBindings => List.unmodifiable(_bindingCache.values);
  ...
  Future<CapabilityResult> invoke(
    ...
    if (_isCircuitBreakerOpen(capabilityId)) {
      ...
      return CapabilityResult.fail(
        const RuntimeError(code: 'UNAVAILABLE', message: 'Circuit breaker open'),
      );
    }
    ...
    if (!_checkPermission(binding, callerPermission)) {
      return CapabilityResult.fail(
        RuntimeError(code: 'PERMISSION_DENIED', message: 'Caller "${caller.identity}" lacks permission for "$capabilityId"'),
      );
    }

    final effectiveSession = session ?? RuntimeSession(
    ...
    try {
      ...
      final result = await binding.handler.invokeCapability(
        ...
        onTimeout: () => throw TimeoutException('Capability $capabilityId timed out after ${binding.timeoutMs}ms'),
      );
      ...
      if (binding.declaration.maxRetries > 0) {
        return _retryInvoke(capabilityId, params, binding, context, binding.maxRetries);
      }
      return CapabilityResult.fail(
  ...
  Future<CapabilityResult> _retryInvoke(
    ...
    if (remainingRetries <= 0 || context.shouldAbort) {
      return CapabilityResult.fail(
        const RuntimeError(code: 'RETRY_EXHAUSTED', message: 'Max retries exhausted'),
      );
    }

    final policy = const FailurePolicy();
    final delay = policy.retry.delayForAttempt(binding.maxRetries - remainingRetries);
    await Future.delayed(delay);

    try {
      final result = await binding.handler.invokeCapability(capabilityId, params, context);
      _resetCircuitBreaker(capabilityId);
      return result;
    } catch (e) {
      _recordCircuitBreakerFailure(capabilityId);
      return _retryInvoke(capabilityId, params, binding, context, remainingRetries - 1);
    }
  }
  ...
  Future<CapabilityBinding> _resolveBinding(String capabilityId) async {
    ...
    if (pluginId == null) {
      throw const RuntimeError(code: 'NOT_FOUND', message: 'No plugin provides this capability');
    }
    ...
    if (declaration == null) {
      throw const RuntimeError(code: 'NOT_FOUND', message: 'Capability not declared');
    }
  ...
  bool _checkPermission(CapabilityBinding binding, RuntimePermission callerPermission) {
    if (binding.permission == 'auto') return true;
    if (binding.permission == 'deny') return false;
    if (binding.permission == 'confirm') return callerPermission.hasCapability(binding.capabilityId);
  ...
  void _recordCircuitBreakerFailure(String capabilityId) {
    _circuitBreakerFailures[capabilityId] = (_circuitBreakerFailures[capabilityId] ?? 0) + 1;
    final failures = _circuitBreakerFailures[capabilityId]!;
    if (failures >= 5) {
      _circuitBreakerOpenUntil[capabilityId] = DateTime.now().add(const Duration(seconds: 30));
      AppLogger.instance.warning('Circuit breaker opened for "$capabilityId" after $failures failures');
    }
  }
