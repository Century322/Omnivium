...
class MemoryEntry {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'lastAccessedAt': lastAccessedAt?.toIso8601String(), 'accessCount': accessCount,
  };

  factory MemoryEntry.fromJson(Map<String, dynamic> json) => MemoryEntry(
    id: json['id'], category: json['category'], content: json['content'],
...
class AgentMemoryService {
  ...
  Future<void> _loadFromDB() async {
    ...
    if (raw != null) {
      try {
        ...
        _memories = list.map((e) => MemoryEntry.fromJson(e as Map<String, dynamic>)).toList();
      } catch (e) { _memories = []; }
    }
  ...
  Future<void> _saveToDB() async {
    final db = DatabaseService.instance;
    await db.putCache(_memoryKey, jsonEncode(_memories.map((m) => m.toJson()).toList()));
    await db.putCache(_profileKey, jsonEncode(_userProfile));
  }

  Future<void> store(String content, {String category = 'fact', double importance = 0.5}) async {
  ...
  Future<List<MemoryEntry>> retrieve(String query, {int maxResults = 5}) async {
    final embeddingResults = await EmbeddingService.instance.searchSimilar(query, maxResults: maxResults * 2, threshold: 0.3);
    if (embeddingResults.isNotEmpty) {
      final keys = embeddingResults.map((e) => e.key).toSet();
      final matched = _memories.where((m) => keys.any((k) => m.content.toLowerCase().contains(k.toLowerCase()))).toList();
  ...
  Future<String> buildMemoryContext(String query, {int maxTokens = 1000}) async {
  ...
  List<_ExtractionPattern> _getRemotePatterns() {
    final remote = RemoteConfigService.instance.getValue<List<dynamic>>('memory_extraction_patterns');
    if (remote == null || remote.isEmpty) return [];
    return remote.map((item) {
      final m = item as Map<String, dynamic>;
      return _ExtractionPattern(
        RegExp(m['pattern'] as String? ?? '', caseSensitive: m['case_sensitive'] as bool? ?? true),
  ...
  Future<void> extractAndStore(String conversationContent) async {
    ...
    final patterns = <_ExtractionPattern>[
      ...
      _ExtractionPattern(RegExp(r"(?:my name is|i'm|i am) (\w+(?:\s\w+)?)", caseSensitive: false), 'name', 0.9),
      ...
      _ExtractionPattern(RegExp(r"(?:i live in|i'm from|i am from) ([\w\s]+)", caseSensitive: false), 'location', 0.7),
      ...
      _ExtractionPattern(RegExp(r"(?:i like|i love|i prefer) ([\w\s]+)", caseSensitive: false), 'preference', 0.8),
      ...
      _ExtractionPattern(RegExp(r"(?:i don't like|i hate|i dislike) ([\w\s]+)", caseSensitive: false), 'dislike', 0.8),
      _ExtractionPattern(RegExp(r'(?:我的工作|我的职业|我是做)(\S+)'), 'occupation', 0.7),
      _ExtractionPattern(RegExp(r"(?:i work as|i'm a|i am a) ([\w\s]+)", caseSensitive: false), 'occupation', 0.7),
      _ExtractionPattern(RegExp(r'(?:我(?:的)?(?:手机|电话|邮箱|email)(?:是|:|：)\s*(\S+))'), 'contact', 0.6),
      _ExtractionPattern(RegExp(r"(?:my (?:phone|email) (?:is|:)\s*(\S+))", caseSensitive: false), 'contact', 0.6),
    ];
      ...
      try {
        ...
        if (match != null) {
          ...
          if (value != null && value.length < 50) {
            await store('${pattern.category}: $value', category: pattern.category, importance: pattern.importance);
            await updateProfile(pattern.category, value);
          }
        }
      } catch (e, stackTrace) {
        AppLogger.instance.warning('Regex extraction failed for category: ${pattern.category}', error: e, stackTrace: stackTrace);
      }
    }
  }
  ...
  Future<RegExpMatch?> _safeRegexMatch(RegExp regex, String input) async {
    try {
      ...
      AppLogger.instance.warning('Regex execution timed out or failed', error: e);
      return null;
    }
  }
